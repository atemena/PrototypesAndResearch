...
function( sandbox, kinetic, bootstrapCP, bootstrapS, jqueryKnob, p, recentPins, _, canvg, rgbcolor){
	...
	$(".bg").click(function(){
		if (canvas.selectedGroup)
			canvas.selectedGroup.image.deleteSelf();
		canvas.scale($(this).width(), $(this).height());
		canvas.addImage($(this).attr("src"));
	})
	...
	$(function() {
		...
		var sShift= $('#sShift').slider({
			...
			canvas.selectedGroup.shadowEdit({'color':'black', 'offsetX': ev.value, 'offsetY': ev.value})
			}).data('slider'); 
		...
		var sAlpha= $('#sAlpha').slider({
			...
			if(ev.value>0)
				canvas.selectedGroup.shadowEdit({'color':'black', 'opacity': ev.value/100});
			else
				canvas.selectedGroup.shadowEdit({'color':'black', 'opacity': 0});
		}).data('slider'); 
